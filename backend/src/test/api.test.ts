...
async function runApiTestSuite() {
  ...
  try {
    ...
    // 3. Business Validation Error Test
    const invalidBusinessRes = await makeRequest(server, 'POST', '/api/businesses', {
      phone: '+1-555-1234',
      email: 'not-an-email',
    });
    assert(
    ...
    // 4. Customer Validation Error Test
    const invalidCustomerRes = await makeRequest(server, 'POST', '/api/customers', {
      name: '',
      phone: '1',
    });
    assert(
    ...
    // 5. Staff Validation (Non-UUID BusinessId) Test
    const invalidStaffRes = await makeRequest(server, 'POST', '/api/staff', {
    ...
    // 6. Service Validation (0 duration) Test
    const invalidServiceRes = await makeRequest(server, 'POST', '/api/services', {
